...
# User input to choose the directory of the code to be process for code smells
print("Please choose the directory to a java project for analyis")
path = askdirectory(title='Select Folder')

# Collect Metrics from the MLCQ datasets using numpy into numpy arrays
# Long Method code metrics values
...
# Connect to the Java server which run the code to extract the CK Metrics These metrics are saved to a csv file in
# the bin folder We extract these metrics related to the class and method and use them to determine the code smells
# i.e God class and Long Method
...
# Read the csv files related to the metrics and find the distances between them and the values in the dataset to determine the code smells
...
LM_df = LM_df[['file', 'class', 'method', 'Predicted_Values', 'wmc', 'loc', 'maxNestedBlocksQty',
...
GC_df = GC_df[['file', 'class', 'Predicted_Values','wmc','tcc','totalMethodsQty','totalFieldsQty','loc',
              'Predicted_Metrics', 'Euclidean_dist']]
